...
saveTrucksBtn.addEventListener('click', () => {
    const newValue = parseInt(trucksInput.value);
    if (!isNaN(newValue) && newValue >= 0) {
        activeTrucksSpan.textContent = newValue;
        modal.style.display = 'none';
...
voyageForm.addEventListener('submit', async (e) => {
    ...
    const voyage = {
        ...
        societe: document.getElementById('company-selector').value // KIS ou UTA
    };
...
function renderTable(data) {
    ...
    voyagesTable.addEventListener('click', function(e) {
...
exportPDFBtn.addEventListener('click', async () => {
    try {
        ...
        doc.text(`Généré par: TCHIO NGOUMO ALAIN`, margin, 35);
        doc.text(`Fonction: Agent Logistique`, margin, 40);//Kribi, Cameroun 657 60 08 55 / 681 23 33 07 alain.tchio@kis-kribi.org
        doc.text(`Contact: 657 60 08 55 / 681 23 33 07`, margin, 45);
        doc.text(`Email: alain.tchio@kis-kribi.org`, margin, 50);
        doc.text(`Date: ${new Date().toLocaleDateString('fr-FR')}`, margin, 55);
        doc.text(`Nombre de voyages: ${allVoyages.length}`, pageWidth - margin, 35, null, null, 'right');
        doc.text(`Période: ${timeFilter.options[timeFilter.selectedIndex].text}`, pageWidth - margin, 40, null, null, 'right');
